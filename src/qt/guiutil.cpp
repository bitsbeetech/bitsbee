...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bitsbee address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no Bitsbee: URI
...
{
    // Convert bitsbee:// to bitsbee:
    //
    //    Cannot handle this later, because bitsbee:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open bitsbee.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitsbee.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Bitsbee.lnk
...
{
    return GetAutostartDir() / "bitsbee.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a bitsbee.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Bitsbee\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the bitsbee app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add bitsbee app to startup item list
